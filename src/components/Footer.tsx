...
export function Footer() {
  ...
  return (
    <footer className="relative border-t border-black/[0.08] bg-[#f3eee8] text-[#040403]">
      <div className="mx-auto max-w-[1400px] px-6 pb-12 pt-24 md:px-10">
        {/* Top grid */}
            ...
            <ul className="flex flex-col gap-2.5">
              {navItems.map((item) => (
                <li key={item.name}>
                  <Link
                    href={item.href}
                    className="text-[12px] font-medium text-black/50 transition-colors duration-200 hover:text-[#69a65b]"
                  >
            ...
            <div className="flex items-center gap-3">
              {socialLinks.map((s) => (
                <motion.a
                  ...
                  className="flex h-9 w-9 items-center justify-center rounded-full border border-black/[0.12] text-black/40 transition-colors duration-200 hover:border-[#69a65b]/50 hover:text-[#69a65b]"
                >
                  {SOCIAL_ICONS[s.icon]}
                </motion.a>
              ))}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-8">
          ...
          <button
            ...
            className="group flex items-center gap-2 text-[10px] font-mono tracking-[0.2em] text-black/35 uppercase transition-colors hover:text-[#69a65b]"
          >
            BACK TO TOP
              <span className="flex h-6 w-6 items-center justify-center border border-current transition-colors group-hover:border-[#69a65b]">
